...
# Function to calculate packet counts
def calculate_packet_counts(filename):
    ...
    # Optionally, you can calculate the total number of packets
...
@app.route('/api/update-packet-counts', methods=['GET'])
def trigger_update_packet_counts():
    global current_packet_counts
    filename = '/home/kali/Desktop/website/ApplicationDataManager/packet_log.csv'  # Adjust file path
    packet_counts = calculate_packet_counts(filename)
    current_packet_counts = packet_counts
    return jsonify(current_packet_counts)

# Endpoint to get packet counts
@app.route('/api/packet-counts', methods=['GET'])
...
if __name__ == '__main__':
    filename = '/home/kali/Desktop/XXXXx/project/packet_log.csv'  # Adjust file path
