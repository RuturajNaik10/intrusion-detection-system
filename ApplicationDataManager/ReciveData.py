...
def print_packet_info(packet, csv_writer1,csv_writer2,con):
    if IP in packet:
        ...
        #con.send(bytes(str(log_entry_csv),'utf-8'))
        # Log to console and file
        print(log_entry)
        logging.info(log_entry)

def packet_sniffer(con):
    ...
    try:
        while True:
            with open(csv_filename, 'a', newline='') as csv_file1, open('malicious.csv', 'a', newline='') as csv_file2:
                ...
                sniff(prn=lambda pkt: print_packet_info(pkt, csv_writer1,csv_writer2,con), store=False, iface=['eth0', 'lo'], timeout=5)
